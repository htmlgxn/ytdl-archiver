...
class TestPlaylistArchiver:
    ...
    def test_get_playlist_info(self, config):
        ...
        with patch('ytdl_archiver.core.archive.yt_dlp.YoutubeDL') as mock_ydl:
            ...
            mock_ydl_instance.extract_info.return_value = {"id": "test_playlist", "title": "Test"}
            
            result = archiver._get_playlist_info("https://www.youtube.com/playlist?list=test")
    ...
    def test_process_playlist_basic(self, mock_downloader, config, temp_dir, sample_playlist_data):
        """Test basic playlist processing."""
        # Setup mocks
        mock_downloader_instance = Mock()
        mock_downloader.return_value = mock_downloader_instance
        mock_downloader_instance.download_video_with_config.return_value = sample_playlist_data["entries"][0]
        
        # Create archiver
        archiver = PlaylistArchiver(config)
        
        # Mock the _get_playlist_info method
        with patch.object(archiver, '_get_playlist_info', return_value=sample_playlist_data):
            # Process playlist
            archiver.process_playlist("PLOgg6_QCO8CeFd55aR1RgzSQOOWhR12uj", "TestPlaylist")
    ...
    def test_process_playlist_none_entries(self, config):
        ...
        with patch.object(archiver, '_get_playlist_info', return_value=playlist_with_none):
            # Should not raise exception
